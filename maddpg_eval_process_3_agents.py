...
#  ------------------- removed eval ------------------
			...
			for i in range(env.n):
				...
				if replayMemory.size()>int(args['minibatch_size']):
					...
					a_temp1 = np.dstack((a_temp1, z))
					a_temp1 = np.transpose(np.asarray(a_temp1), (0,2,1))
 # , axis=0
					# print(tem)
					# print(tem.shape)
					# print(a_temp1.shape)
					...
					a_batch1 = np.transpose(np.asarray(a_batch), (0,2,1))
					a_batch1 = np.dstack((a_batch1, z))

					# a_for_critic = np.asarray([x.flatten() for x in a_temp])
					# ------------------------------- changed --------------
					a_for_critic = np.asarray([x.flatten() for x in a_temp1]) # changed
					# print(a_for_critic.shape)
					# ------------------------------- changed --------------
					
					# --------------------------
					
					# print(np.zeros(shape= (a_for_critic.shape[0], 2)).shape)
					# a_for_critic= np.column_stack((a_for_critic, np.zeros(shape= (a_for_critic.shape[0], 2)))) #, axis=0
					# print(a_batch.shape)
					# # a_batch= np.column_stack((a_batch, np.zeros(shape= (a_batch.shape[0], 2))))
					# # print(a_for_critic)
					# print(a_for_critic.shape)
					# print(a_batch.shape)
					# -------------------------
					...
					critic.train(s_batch_i,np.asarray([x.flatten() for x in a_batch1]),np.reshape(np.array(yi),(int(args['minibatch_size']),2)))
					# ------------------------------- changed --------------
					...
					a_temp1 = np.transpose(np.asarray(actions_pred), (1,2,0))
					# print(a_temp1.shape)
					a_temp1 = np.dstack((a_temp1, z))
					a_temp1 = np.transpose(np.asarray(a_temp1), (0,2,1))					
					# ------------------------------- changed --------------
