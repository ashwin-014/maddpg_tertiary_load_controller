...
class MultiAgent(gym.Env):
    ...
    def find_sum(self, pow_set, action_load_alloc):
        diff = np.zeros(shape= len(pow_set)-1)

        # --------------------------- removed print ------------------------------------
        # print("\npow_set", pow_set)
        # --------------------------- removed print ------------------------------------

        for i in range(len(pow_set)-1):
            diff[i] = np.abs(np.sum(pow_set[i+1]) - 4*action_load_alloc[0])

            # --------------------------- removed print ------------------------------------
            # print("\ni:",i,"\nsum:",np.sum(pow_set[i+1]),"alloc:",4*action_load_alloc[0]) #"shape:", pow_set[i+1].shape,
            # --------------------------- removed print ------------------------------------

        print("alloc_row_num",np.argmin(diff)+1, "\nalloc:", pow_set[np.argmin(diff)+1])
    ...
    def _get_reward(self,agent, action):
        ...
        if (((agent.amount_allocated - agent.ndt)) <= MAIN_POWER_MAX_LIMIT) and (((agent.amount_allocated - agent.ndt))>=MAIN_POWER_MIN_LIMIT):
            agent.cost += 1*agent.delay + self.unit_price * 50*(agent.amount_allocated - agent.ndt)
        else:
            agent.cost+=101
        
        agent.rew += (1 - (agent.time_step_not_fully_allocated*100)  - (agent.cost) - np.abs((int(action[1]) - agent.allocated_load.shape[0])*10) - np.abs(action[0] - agent.amount_allocated)*10)
        ...
        if self.time_is_over:
            if not agent.failed_allocate:
                ...
                agent.rew -= 1.50*(len(agent.deq_for_unalloc[0])+len(agent.deq_for_unalloc[1])+len(agent.deq_for_unalloc[2])+len(agent.deq_for_unalloc[3]))
